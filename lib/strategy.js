 ...
 * Options:
 *
 *   - `name`              Passport oauth2 name (default: `oauth2`)
...
function OAuth2Strategy(options, verify) {
  ...
  } else if (options.authorizationURL) {
    this._key = this.name + ':' + url.parse(options.authorizationURL).hostname;
  } else {
    this._key = this.name + ':' + '';
  }
...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  options.authorizationURL = options.authorizationURL || this._authorizationURL;
  options.tokenURL = options.tokenURL || this._tokenURL;
  options.clientID = options.clientID || this._clientID;
  options.clientSecret = options.clientSecret || this._clientSecret;
